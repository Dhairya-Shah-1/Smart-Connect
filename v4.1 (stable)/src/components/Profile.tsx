...
import { User, LogOut, Mail, Calendar, MapPin as MapPinIcon } from 'lucide-react';
...
export function Profile({ onLogout }: ProfileProps) {
  ...
  useEffect(() => {
    ...
    const userReports = reports.filter((r: any) => r.userEmail === currentUser.email);
    setTotalReports(userReports.length);
    setResolvedReports(userReports.filter((r: any) => r.status === 'resolved').length);
  }, []);
    ...
    <div className={`h-full overflow-y-auto ${isDark ? 'bg-slate-900' : 'bg-slate-50'} p-4 md:p-6`}>
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div className={`rounded-xl shadow-lg border overflow-hidden ${
          ...
          <div className={`h-24 md:h-32 ${isDark ? 'bg-gradient-to-br from-slate-700 to-slate-800' : 'bg-gradient-to-br from-blue-800 to-blue-900'}`}></div>
